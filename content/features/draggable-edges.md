...
| `connectionPreprocessor`    | `(request: AddEdgeRequest) => AddEdgeRequest`                                          | Applies modification to the edge about to be reattached. `null` means that connection is disallowed.                     | no       | `(request) => request`                           |
