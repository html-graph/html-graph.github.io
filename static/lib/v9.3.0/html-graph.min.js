...
var ae = (e, t, n) => {
...
}, Nt = (e, t, n) => e === "nearest-connectable-port" ? new lt(t, n) : typeof e == "number" ? new ut(e) : new ut(void 0), Y = (e) => {
	if (typeof e == "function") return e;
	switch (e.type) {
...
}, Pt = (e) => e.button === 0 && e.ctrlKey, X = (e) => e.button === 0, Z = (e) => e.button === 0 && !e.ctrlKey, Ft = (e) => {
	...
	apply(e, t) {
		...
		for (let i = 0; i < r; i++) {
			let a = n[i];
			for (let o = i + 1; o < r; o++) {
				let r = n[o], i = e.get(a), s = e.get(r), c = this.distanceVectorGenerator.create(i, s), l = zt({
					coefficient: 1,
	...
	calculateNodeRepulsiveForce(e) {
		let t = this.distanceVectorGenerator.create(e.sourceCoords, e.targetCoords), n = zt({
	...
	stopVelocity;
	constructor(e) {
		this.maxIterations = e.maxIterations, this.dtSec = e.dtSec, this.nodeMass = e.nodeMass, this.edgeEquilibriumLength = e.edgeEquilibriumLength, this.edgeStiffness = e.edgeStiffness, this.stopVelocity = e.stopVelocity, this.distanceVectorGenerator = new Rt(e.rand), this.nodeForcesApplicationStrategy = Wt({
			...
			nodeMass: e.nodeMass
		}), this.fillerLayoutAlgorithm = new Gt({
			rand: e.rand,
	...
	calculateCoordinates(e) {
		...
		for (let e = 0; e < this.maxIterations && !(new Lt(t, r, {
			distanceVectorGenerator: this.distanceVectorGenerator,
	...
	constructor(e, t) {
		this.tree = e;
		let n = t.spaceAroundRadius, r = new Jt({ spaceAroundRadius: n });
	...
	calculateCoordinates(e) {
		let t = /* @__PURE__ */ new Map(), n = new qt(e.graph, this.params.nextLayerNodesResolver).generate(), r = 0;
		return n.forEach((e) => {
			t.set(e.root.nodeId, {
				x: r,
				y: 0
			});
			let n = new Yt(e, { spaceAroundRadius: this.params.layerSpace / 2 }).generate(), i = [e.root];
			for (; i.length > 0;) {
				let e = [];
	...
	fillerLayoutAlgorithm;
	constructor(e) {
		this.stopVelocity = e.stopVelocity, this.maxTimeDeltaSec = e.maxTimeDeltaSec, this.nodeMass = e.nodeMass, this.edgeEquilibriumLength = e.edgeEquilibriumLength, this.edgeStiffness = e.edgeStiffness, this.distanceVectorGenerator = new Rt(e.rand), this.nodeForcesApplicationStrategy = Wt({
			distanceVectorGenerator: this.distanceVectorGenerator,
			nodeCharge: e.nodeCharge,
			maxForce: e.maxForce,
			nodeForceCoefficient: e.nodeForceCoefficient,
	...
	switch (e?.type) {
		...
		case "hierarchical": return new Xt({
			layerWidth: e.layerWidth ?? Q.hierarchicalLayout.layerWidth,
			layerSpace: e.layerSpace ?? Q.hierarchicalLayout.layerSpace,
			transform: on(e.transform),
			nextLayerNodesResolver: sn(e.nextLayerNodesResolver)
		});
		default: {
			let t = tn(e?.seed ?? x.seed), n = nn(t[0], t[1], t[2], t[3]);
			return new Kt({
...
}), un = (e) => e.applyOn.type === "topologyChangeSchedule" ? e.applyOn.schedule : V, dn = (e) => {
	let { canvasDefaults: t } = e, n = e.hasLayout ? un(e.layoutParams) : V;
	return { focus: {
...
}, pn = (e) => e instanceof SVGElement ? e : fn(e?.radius ?? 1.5, e?.color ?? "#d8d8d8"), mn = (e) => {
	let t = e?.tileDimensions;
	return {
		tileWidth: t?.width ?? 25,
		tileHeight: t?.height ?? 25,
		renderer: pn(e?.renderer ?? {}),
	...
	let r = e?.connectionAllowedVerifier ?? bn.connectionAllowedVerifier;
	return {
		connectionTypeResolver: e?.connectionTypeResolver ?? bn.connectionTypeResolver,
		...
		connectionPreprocessor: e?.connectionPreprocessor ?? bn.connectionPreprocessor,
		mouseDownEventVerifier: e?.mouseDownEventVerifier ?? Z,
		mouseUpEventVerifier: e?.mouseUpEventVerifier ?? X,
	...
	}, r = e?.connectionAllowedVerifier ?? Sn.connectionAllowedVerifier;
	return {
		connectionPreprocessor: e?.connectionPreprocessor ?? Sn.connectionPreprocessor,
		connectionAllowedVerifier: r,
		mouseDownEventVerifier: e?.mouseDownEventVerifier ?? Z,
		mouseUpEventVerifier: e?.mouseUpEventVerifier ?? X,
	...
	return n = t === void 0 ? (e) => e.nextTransform : Array.isArray(t) ? En(t.map((e) => Dn(e))) : Dn(t), {
	...
	rectangleElement: e?.rectangleElement ?? An(),
	mouseDownEventVerifier: e?.mouseDownEventVerifier ?? Pt,
	mouseUpEventVerifier: e?.mouseUpEventVerifier ?? X,
	...
	switch (e?.type) {
		case "custom": return e.instance;
		default: {
			let t = tn(e?.seed ?? x.seed);
			return new en({
				rand: nn(t[0], t[1], t[2], t[3]),
...
}, Nn = { staticNodeResolver: () => !1 }, Pn = (e) => ({
	algorithm: Mn(e?.algorithm ?? {}),
	staticNodeResolver: e?.staticNodeResolver ?? Nn.staticNodeResolver,
	onBeforeApplied: e?.events?.onBeforeApplied ?? S,
	onAfterApplied: e?.events?.onAfterApplied ?? S
}), Fn = (e, t) => {
	...
	canvas;
	constructor(e, t) {
		this.element = e, this.params = t, this.pointInsideVerifier = new ne(this.element, this.window), this.viewportStore = new s(this.element), this.layers = new h(this.element), this.htmlView = this.createHtmlView(this.layers.main, this.graphStore, this.viewportStore), this.graphControllerParams = Ft(this.params.canvasDefaults), this.graphController = new Ge(this.graphStore, this.htmlView, this.graphControllerParams), this.layoutParams = ln(this.params.layout.config);
		let n = dn({
		...
		if (this.canvas = new r(o, a, this.graphController, i), this.params.background.enabled && _t.configure(this.canvas, mn(this.params.background.config), this.layers.background), this.params.nodeResizeReactiveEdges.enabled && C.configure(this.canvas), this.params.userSelectableEdges.enabled) {
			let e = hn(this.params.userSelectableEdges.config);
		...
		if (this.params.userSelectableNodes.enabled) {
			let e = gn(this.params.userSelectableNodes.config);
		...
		if (this.params.userSelectableCanvas.enabled) {
			let e = _n(this.params.userSelectableCanvas.config);
		...
		if (this.params.draggableNodes.enabled) {
			let e = vn(this.params.draggableNodes.config);
			this.params.animatedLayout.enabled && (e = yn(e, this.animationStaticNodes)), dt.configure(this.canvas, this.layers.main, this.window, this.pointInsideVerifier, this.eventTagger, e);
		}
		if (this.params.userConnectablePorts.enabled) {
			let e = xn(this.params.userConnectablePorts.config, this.graphControllerParams.edges.shapeFactory, this.canvas.graph);
		...
		if (this.params.userDraggableEdges.enabled) {
			let e = Cn(this.params.userDraggableEdges.config, this.canvas.graph);
		...
		if (this.params.virtualScroll.enabled ? pt.configure(this.canvas, this.layers.main, this.window, On(this.params.userTransformableViewport.config), this.boxRenderingTrigger, this.pointInsideVerifier, this.eventTagger, kn(this.params.virtualScroll.config)) : this.params.userTransformableViewport.enabled && ft.configure(this.canvas, this.layers.main, this.window, this.pointInsideVerifier, this.eventTagger, On(this.params.userTransformableViewport.config)), this.params.rectangularSelection.enabled && Mt.configure(this.canvas, this.layers.main, this.layers.overlayRectangularSelection, this.pointInsideVerifier, this.eventTagger, this.window, jn(this.params.rectangularSelection.config)), this.params.layout.enabled && Ct.configure(this.canvas, this.layoutParams), this.params.animatedLayout.enabled) {
			let e = Pn(this.params.animatedLayout.config);
			this.params.draggableNodes.enabled && (Fn(this.canvas, this.animationStaticNodes), e = In(e, this.animationStaticNodes)), Et.configure(this.canvas, e, this.window);
		}
	...
	createHtmlView(e, t, n) {
		let r = new Xe(t, n, e);
		return this.params.virtualScroll.enabled && (r = new Qe(r, t, this.boxRenderingTrigger, Ln(this.params.virtualScroll.config))), r = new $e(r, t), r;
	}
}, zn = class {
	element;
	used = !1;
	...
	build() {
		...
		return new Rn(this.element, t).canvas;
	}
};
//#endregion
export { je as BezierEdgeShape, zn as CanvasBuilder, e as CanvasBuilderError, l as CanvasError, I as ConnectionCategory, z as DirectEdgeShape, t as EventSubject, Ve as InteractiveEdgeError, He as InteractiveEdgeShape, Ue as MidpointEdgeShape, Pe as OrthogonalEdgeShape, Me as StraightEdgeShape };
