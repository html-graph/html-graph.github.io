//#region src/edges/connection-category.ts
var e = /* @__PURE__ */ function(e) {
	...
	attachNode(e) {
		...
		r(t.element), this.attachedNodeIds.add(e), this.container.appendChild(t.element), this.updateNodePosition(e), this.updateNodePriority(e), t.element.style.visibility = "visible";
	}
	detachNode(e) {
		let t = this.graphStore.getNode(e);
	...
	attachEdge(e) {
		let t = this.graphStore.getEdge(e).payload.shape.svg;
	...
	updateEdgeShape(e) {
		...
		let n = this.graphStore.getEdge(e).payload.shape.svg;
		this.edgeIdToElementMap.set(e, n), this.container.appendChild(n);
	}
	renderEdge(t) {
		let n = this.graphStore.getEdge(t), r = this.graphStore.getPort(n.from), i = this.graphStore.getPort(n.to), a = r.element.getBoundingClientRect(), o = i.element.getBoundingClientRect(), s = this.host.getBoundingClientRect(), c = this.viewportStore.getViewportMatrix().scale, l = this.createEdgeRenderPort(r, a, s, c), u = this.createEdgeRenderPort(i, o, s, c), d = e.Line;
		r.element === i.element ? d = e.PortCycle : r.nodeId === i.nodeId && (d = e.NodeCycle), n.payload.shape.render({
			from: l,
			to: u,
			category: d
		});
	...
	constructor(e, t, n, r) {
		this.htmlView = e, this.graphStore = t, this.trigger = n, this.params = r, this.renderingBox = new o(this.graphStore), this.trigger.subscribe(this.updateViewport);
	}
	attachNode(e) {
	...
	onBeforeDestroy;
	constructor(e, t, n, r) {
		this.graph = e, this.viewport = t, this.graphController = n, this.viewportController = r, [this.beforeDestroyEmitter, this.onBeforeDestroy] = u();
	}
	addNode(e) {
	...
	onBeforeClear;
	constructor() {
		[this.afterNodeAddedEmitter, this.onAfterNodeAdded] = u(), [this.afterNodeUpdatedEmitter, this.onAfterNodeUpdated] = u(), [this.afterNodePriorityUpdatedEmitter, this.onAfterNodePriorityUpdated] = u(), [this.beforeNodeRemovedEmitter, this.onBeforeNodeRemoved] = u(), [this.afterPortAddedEmitter, this.onAfterPortAdded] = u(), [this.afterPortUpdatedEmitter, this.onAfterPortUpdated] = u(), [this.beforePortRemovedEmitter, this.onBeforePortRemoved] = u(), [this.afterEdgeAddedEmitter, this.onAfterEdgeAdded] = u(), [this.afterEdgeShapeUpdatedEmitter, this.onAfterEdgeShapeUpdated] = u(), [this.afterEdgeUpdatedEmitter, this.onAfterEdgeUpdated] = u(), [this.afterEdgePriorityUpdatedEmitter, this.onAfterEdgePriorityUpdated] = u(), [this.beforeEdgeRemovedEmitter, this.onBeforeEdgeRemoved] = u(), [this.beforeClearEmitter, this.onBeforeClear] = u();
	}
	hasNode(e) {
	...
	getNode(e) {
		...
		if (t === void 0) throw new p(m.accessNonexistingNode(e));
		return t;
	}
	addNode(e) {
		if (this.hasNode(e.id)) throw new p(m.addNodeWithExistingId(e.id));
		let t = this.findNodeIdByElement(e.element);
		if (t !== void 0) throw new p(m.addNodeWithElementInUse(e.id, t));
	...
	updateNode(e, t) {
		if (!this.hasNode(e)) throw new p(m.updateNonexistentNode(e));
	...
	removeNode(e) {
		if (!this.hasNode(e)) throw new p(m.removeNonexistentNode(e));
	...
	getPort(e) {
		...
		if (t === void 0) throw new p(m.accessNonexistentPort(e));
		return t;
	}
	addPort(e) {
		if (this.hasPort(e.id)) throw new p(m.addPortWithExistingId(e.id));
		if (!this.hasNode(e.nodeId)) throw new p(m.addPortToNonexistentNode(e.id, e.nodeId));
		this.ports.set(e.id, {
			element: e.element,
	...
	updatePort(e, t) {
		if (!this.hasPort(e)) throw new p(m.updateNonexistentPort(e));
	...
	getNodePortIds(e) {
		let t = this.nodes.get(e);
		if (t === void 0) throw new p(m.accessPortsOfNonexistentNode(e));
		return Array.from(t.ports.keys());
	}
	removePort(e) {
		if (!this.hasPort(e)) throw new p(m.removeNonexistentPort(e));
	...
	getEdge(e) {
		...
		if (t === void 0) throw new p(m.accessNonexistentEdge(e));
		return t;
	}
	...
	updateEdge(e, t) {
		...
		if (t.from !== void 0 || t.to !== void 0) {
			...
			let n = this.getEdge(e), r = n.payload;
	...
	removeEdge(e) {
		if (!this.hasEdge(e)) throw new p(m.removeNonexistentEdge(e));
	...
	getPortIncomingEdgeIds(e) {
		...
		if (t === void 0) throw new p(m.accessEdgesForNonexistentPort(e));
		return Array.from(t);
	}
	...
	addEdgeInternal(e) {
		...
		}), this.edgesElementsMap.set(e.shape.svg, e.id), e.from === e.to ? this.portCycleEdges.get(e.from).add(e.id) : (this.portOutgoingEdges.get(e.from).add(e.id), this.portIncomingEdges.get(e.to).add(e.id));
	}
	removeEdgeInternal(e) {
		let { from: t, to: n, payload: r } = this.getEdge(e);
		this.portCycleEdges.get(t).delete(e), this.portCycleEdges.get(n).delete(e), this.portIncomingEdges.get(t).delete(e), this.portIncomingEdges.get(n).delete(e), this.portOutgoingEdges.get(t).delete(e), this.portOutgoingEdges.get(n).delete(e), this.edges.delete(e), this.edgesElementsMap.delete(r.shape.svg);
	}
}, g = (e) => ({
	scale: 1 / e.scale,
	x: -e.x / e.scale,
	...
	constructor(e) {
		this.host = e, [this.afterUpdateEmitter, this.onAfterUpdated] = u(), [this.beforeUpdateEmitter, this.onBeforeUpdated] = u(), [this.afterResizeEmitter, this.onAfterResize] = u(), this.observer.observe(this.host);
	}
	getViewportMatrix() {
	...
	patchViewportMatrix(e) {
		...
		}, this.beforeUpdateEmitter.emit(), this.contentMatrix = g(this.viewportMatrix), this.afterUpdateEmitter.emit();
	}
	patchContentMatrix(e) {
		this.contentMatrix = {
		...
		}, this.beforeUpdateEmitter.emit(), this.viewportMatrix = g(this.contentMatrix), this.afterUpdateEmitter.emit();
	}
	getDimensions() {
	...
	createContentCoords(e) {
		return v(this.viewportMatrix, e);
	}
	createViewportCoords(e) {
		return v(this.contentMatrix, e);
	}
	destroy() {
	...
	for (; n !== null;) {
		...
		if (e.findNodeIdByElement(n) !== void 0) return { status: "nodeEncountered" };
		n = n.parentElement;
	}
...
var ne = (e, t) => {
	...
	let n = e.linePoint, r = t.linePoint, i = r.x - n.x >= 0, a = e.dirX >= 0;
	if (a === t.dirX >= 0) {
		let o = a === i, s = {
			x: (n.x + r.x) / 2,
			y: (n.y + r.y) / 2
	...
	let n = e.linePoint, r = t.linePoint, i = r.y - n.y >= 0, a = e.dirY >= 0;
	if (a === t.dirY >= 0) {
		let o = a === i, s = {
			x: (n.x + r.x) / 2,
			y: (n.y + r.y) / 2
	...
	midpoint;
	constructor(e) {
		let { from: t, to: n, hasSourceArrow: r, hasTargetArrow: i, arrowLength: a, arrowOffset: o, fromDir: s, toDir: c, roundness: l } = e, u = r ? D({
			x: t.x + a,
			y: t.y
		}, s, t) : t, d = i ? D({
			x: n.x - a,
		...
		this.path = M(h.points, l), this.midpoint = h.midpoint;
	}
}, ge = class {
	path;
	midpoint;
	...
	arrowRenderer;
	constructor(e) {
		this.params = e, [this.afterRenderEmitter, this.onAfterRender] = u(), this.arrowRenderer = this.params.arrowRenderer, this.svg = se(e.color), this.svg.appendChild(this.group), this.line = ce(e.width), this.group.appendChild(this.line), e.hasSourceArrow && (this.sourceArrow = j(), this.group.appendChild(this.sourceArrow)), e.hasTargetArrow && (this.targetArrow = j(), this.group.appendChild(this.targetArrow));
	}
	render(t) {
		...
		let c = Se(t.from.direction), l = Se(t.to.direction), u = {
			x: -l.x,
			y: -l.y
		}, d;
		t.category === e.PortCycle ? (d = this.params.createCyclePath, u = c) : d = t.category === e.NodeCycle ? this.params.createDetourPath : this.params.createLinePath;
		let f = d(o, s, c, l);
		...
		let m = null;
		this.targetArrow && (m = this.arrowRenderer({
			direction: u,
			shift: s,
			arrowLength: this.params.arrowLength
	...
	switch (e.type) {
		case "triangle": return Ce({ radius: e.radius ?? N.polygonArrowRadius });
		case "arc": return we({ radius: e.radius ?? N.circleArrowRadius });
		default: return Te({
			smallRadius: e.smallRadius ?? N.wedgeArrowSmallRadius,
			angle: e.angle ?? N.wedgeArrowAngle,
			radius: e.radius ?? N.wedgeArrowRadius
		});
	}
}, Ee = class {
	svg;
	...
	constructor(e) {
		this.arrowLength = e?.arrowLength ?? N.arrowLength, this.curvature = e?.curvature ?? N.curvature, this.portCycleRadius = e?.cycleRadius ?? N.cycleRadius, this.portCycleSmallRadius = e?.smallCycleRadius ?? N.smallCycleRadius, this.detourDirection = e?.detourDirection ?? N.detourDirection, this.detourDistance = e?.detourDistance ?? N.detourDistance, this.hasSourceArrow = e?.hasSourceArrow ?? N.hasSourceArrow, this.hasTargetArrow = e?.hasTargetArrow ?? N.hasTargetArrow, this.pathShape = new P({
			color: e?.color ?? N.color,
			width: e?.width ?? N.width,
			arrowRenderer: F(e?.arrowRenderer ?? {}),
			arrowLength: this.arrowLength,
			hasSourceArrow: this.hasSourceArrow,
			hasTargetArrow: this.hasTargetArrow,
...
}, De = (e) => Math.cos(e) > 0 ? 0 : Math.PI, Oe = class {
	svg;
	group;
	line;
	sourceArrow;
	...
	createDetourPath = (e, t, n, r) => new ye({
		from: e,
		to: t,
		fromDir: n,
		toDir: r,
	...
	constructor(e) {
		this.arrowLength = e?.arrowLength ?? N.arrowLength, this.arrowOffset = e?.arrowOffset ?? N.arrowOffset, this.cycleSquareSide = e?.cycleSquareSide ?? N.cycleSquareSide;
		let t = e?.roundness ?? N.roundness;
		this.roundness = Math.min(t, this.arrowOffset, this.cycleSquareSide / 2), this.detourDistance = e?.detourDistance ?? N.detourDistance, this.hasSourceArrow = e?.hasSourceArrow ?? N.hasSourceArrow, this.hasTargetArrow = e?.hasTargetArrow ?? N.hasTargetArrow, this.pathShape = new P({
			color: e?.color ?? N.color,
			width: e?.width ?? N.width,
			arrowRenderer: F(e?.arrowRenderer ?? {}),
			arrowLength: this.arrowLength,
	...
	render(e) {
		...
		this.pathShape.render({
			...
			from: {
				...t,
				direction: De(t.direction)
			},
			to: {
				...n,
				direction: De(n.direction)
			}
		});
	}
}, ke = class {
	...
	createDetourPath = (e, t, n, r) => new fe({
		from: e,
		to: t,
		fromDir: n,
		toDir: r,
	...
	constructor(e) {
		this.arrowLength = e?.arrowLength ?? N.arrowLength, this.arrowOffset = e?.arrowOffset ?? N.arrowOffset, this.cycleSquareSide = e?.cycleSquareSide ?? N.cycleSquareSide;
		let t = e?.roundness ?? N.roundness;
		this.roundness = Math.min(t, this.arrowOffset, this.cycleSquareSide / 2), this.detourDirection = e?.detourDirection ?? N.detourDirection, this.detourDistance = e?.detourDistance ?? N.detourDistance, this.hasSourceArrow = e?.hasSourceArrow ?? N.hasSourceArrow, this.hasTargetArrow = e?.hasTargetArrow ?? N.hasTargetArrow, this.pathShape = new P({
			color: e?.color ?? N.color,
			width: e?.width ?? N.width,
			arrowRenderer: F(e?.arrowRenderer ?? {}),
			arrowLength: this.arrowLength,
...
}, Ae = Math.PI / 2, je = (e) => Math.sin(e) > 0 ? Ae : -Ae, Me = class {
	svg;
	group;
	line;
	sourceArrow;
	...
	createDetourPath = (e, t, n, r) => new xe({
		from: e,
		to: t,
		fromDir: n,
		toDir: r,
...
}, Pe = (e) => typeof e == "number" ? () => e : e === "box" ? Ne : e, Fe = N.portOffset, I = class {
	svg;
	group = document.createElementNS("http://www.w3.org/2000/svg", "g");
	line;
	sourceArrow = null;
	...
	arrowRenderer;
	constructor(e) {
		[this.afterRenderEmitter, this.onAfterRender] = u(), this.color = e?.color ?? N.color, this.width = e?.width ?? N.width, this.arrowLength = e?.arrowLength ?? N.arrowLength, this.arrowRenderer = F(e?.arrowRenderer ?? {}), this.sourceOffsetFn = Pe(e?.sourceOffset ?? Fe), this.targetOffsetFn = Pe(e?.targetOffset ?? Fe), this.svg = se(this.color), this.svg.appendChild(this.group), this.line = ce(this.width), this.group.appendChild(this.line), e?.hasSourceArrow && (this.sourceArrow = j(), this.group.appendChild(this.sourceArrow)), e?.hasTargetArrow && (this.targetArrow = j(), this.group.appendChild(this.targetArrow));
	}
	render(e) {
	...
	constructor(t, n) {
		...
		let r = n?.distance ?? N.interactiveWidth;
	...
	svg;
	constructor(e, t) {
		this.baseShape = e, this.midpointElement = t, this.svg = this.baseShape.svg, this.group = this.baseShape.group, this.line = this.baseShape.line, this.sourceArrow = this.baseShape.sourceArrow, this.targetArrow = this.baseShape.targetArrow, this.onAfterRender = this.baseShape.onAfterRender, this.svg.append(this.midpointElement), this.baseShape.onAfterRender.subscribe((e) => {
	...
	htmlView;
	params;
	nodeIdGenerator = new L((e) => this.graphStore.hasNode(e));
	portIdGenerator = new L((e) => this.graphStore.hasPort(e));
	edgeIdGenerator = new L((e) => this.graphStore.hasEdge(e));
	onAfterNodeAdded = (e) => {
	...
	let r = new h();
	return new d(new He(r), new Ke(t), new Ue(r, new a(r, t, e), {
		nodes: {
			centerFn: re,
			priorityFn: () => 0
		},
	...
	onPortMouseDown = (e) => {
		...
		let n = e.currentTarget, r = this.canvas.graph.findPortIdsByElement(n)[0];
		this.params.onPointerDownVerifier(r, {
			x: t.clientX,
			y: t.clientY
		}) && (this.eventTagger.tag(e, z), this.window.addEventListener("mousemove", this.onWindowMouseMove, { passive: !0 }), this.window.addEventListener("mouseup", this.onWindowMouseUp, { passive: !0 }));
	};
	onWindowMouseMove = (e) => {
	...
	onPortTouchStart = (e) => {
		...
		this.params.onPointerDownVerifier(i, {
			x: n.clientX,
			y: n.clientY
		}) && (this.eventTagger.tag(e, z), this.window.addEventListener("touchmove", this.onWindowTouchMove, { passive: !0 }), this.window.addEventListener("touchend", this.onWindowTouchFinish, { passive: !0 }), this.window.addEventListener("touchcancel", this.onWindowTouchFinish, { passive: !0 }));
	};
	onWindowTouchMove = (e) => {
		let t = e.touches[0];
	...
	tag(e, t) {
		let n = e, r = n[B];
		r === void 0 ? n[B] = /* @__PURE__ */ new Set([t]) : r.add(t);
	}
}, V = Symbol(), H = class {
	window;
	pointInsideVerifier;
	...
	onMouseDown = (e) => {
		this.eventTagger.has(e, z) || this.params.mouseDownEventVerifier(e) && (w(this.element, this.params.shiftCursor), this.window.addEventListener("mousemove", this.onWindowMouseMove, { passive: !0 }), this.window.addEventListener("mouseup", this.onWindowMouseUp, { passive: !0 }), this.startRegisteredTransform());
	};
	onWindowMouseMove = (e) => {
	...
	onTouchStart = (e) => {
		if (!this.eventTagger.has(e, z)) {
			if (this.prevTouches !== null) {
				this.prevTouches = W(e);
				return;
			}
			this.prevTouches = W(e), this.window.addEventListener("touchmove", this.onWindowTouchMove, { passive: !0 }), this.window.addEventListener("touchend", this.onWindowTouchFinish, { passive: !0 }), this.window.addEventListener("touchcancel", this.onWindowTouchFinish, { passive: !0 }), this.startRegisteredTransform();
		}
	};
	onWindowTouchMove = (e) => {
		let t = W(e);
	...
	tryCreateConnection(e) {
		...
		let n = U(this.edgeInProgress, t), r = this.params.connectionAllowedVerifier(n), i = this.params.connectionPreprocessor({
			from: n.from,
			to: n.to
		});
		r && i !== null ? (this.canvas.graph.onAfterEdgeAdded.subscribe(this.onEdgeCreated), this.canvas.addEdge(i), this.canvas.graph.onAfterEdgeAdded.unsubscribe(this.onEdgeCreated)) : this.params.onEdgeCreationPrevented(n);
	}
	moveDraggingPort(e) {
	...
	tryCreateConnection(e) {
		...
		let { from: n, to: r } = U(this.edgeInProgress, t), i = this.draggingEdgePayload, a = {
			id: i.id,
			from: n,
			to: r,
			shape: i.shape,
	...
	constructor(e, t, n) {
		this.win = n, this.applier = new gt(e, t.algorithm, {
			staticNodeResolver: t.staticNodeResolver,
			onBeforeApplied: t.onBeforeApplied,
			onAfterApplied: t.onAfterApplied
		}), new ht(this.win, this.step);
	}
	static configure(t, n, r) {
		new e(t, n, r);
	}
	...
	constructor(e, t, n, r, i) {
		this.canvas = e, this.window = t, this.pointInsideVerifier = n, this.eventTagger = r, this.onNodeSelected = i.onNodeSelected, this.configurator = new H(this.window, this.pointInsideVerifier, {
			onSelected: (e, t) => {
	...
	constructor(e, t, n, r, i) {
		this.canvas = e, this.window = t, this.pointInsideVerifier = n, this.eventTagger = r, this.onEdgeSelected = i.onEdgeSelected, this.configurator = new H(this.window, this.pointInsideVerifier, {
			onSelected: (e, t) => {
	...
	eventHandledTag = V;
	constructor(e, t, n, r, i, a) {
		this.canvas = e, this.element = t, this.window = n, this.pointInsideVerifier = r, this.eventTagger = i, this.onCanvasSelected = a.onCanvasSelected, this.configurator = new H(this.window, this.pointInsideVerifier, {
			onSelected: (e, t) => {
	...
	background = G();
	main = G();
	overlayConnectablePorts = St();
	overlayDraggableEdges = St();
	host = xt();
	constructor(e) {
...
}), J = () => {}, wt = (e, t, n) => e === "closest-connectable-port" || e === "nearest-connectable-port" ? new et(t, n) : new tt(e), Y = (e) => {
	if (typeof e == "function") return e;
	switch (e.type) {
		case "straight": return () => new ke({
			color: e.color,
			width: e.width,
	...
	return {
		...
		onNodeDragStarted: e.events?.onNodeDragStarted ?? J,
		onNodeDrag: e.events?.onNodeDrag ?? J,
		nodeDragVerifier: t,
	...
	return {
		wheelSensitivity: n,
		onTransformStarted: e?.events?.onTransformStarted ?? J,
		onTransformFinished: e?.events?.onTransformFinished ?? J,
		onBeforeTransformChange: e?.events?.onBeforeTransformChange ?? J,
		onTransformChange: e?.events?.onTransformChange ?? J,
	...
	return {
		...
		draggingPortDirectionResolver: wt(e.dragPortDirection, n, r),
		...
		onAfterEdgeCreated: e.events?.onAfterEdgeCreated ?? J,
		onEdgeCreationInterrupted: e.events?.onEdgeCreationInterrupted ?? J,
		onEdgeCreationPrevented: e.events?.onEdgeCreationPrevented ?? J
	};
}, Z = Object.freeze({
	connectionAllowedVerifier: () => !0,
	connectionPreprocessor: (e) => e,
	...
	return {
		...
		draggingPortDirectionResolver: wt(e.dragPortDirection, t, r)
	};
}, It = (e) => ({
	...
	convergenceVelocity;
	constructor(e) {
		this.maxIterations = e.maxIterations, this.dtSec = e.dtSec, this.nodeMass = e.nodeMass, this.edgeEquilibriumLength = e.edgeEquilibriumLength, this.edgeStiffness = e.edgeStiffness, this.convergenceVelocity = e.convergenceVelocity, this.distanceVectorGenerator = new Bt(e.rand), this.nodeForcesApplicationStrategy = Kt({
	...
	fillerLayoutAlgorithm;
	constructor(e) {
		this.convergenceVelocity = e.convergenceVelocity, this.maxTimeDeltaSec = e.maxTimeDeltaSec, this.nodeMass = e.nodeMass, this.edgeEquilibriumLength = e.edgeEquilibriumLength, this.edgeStiffness = e.edgeStiffness, this.distanceVectorGenerator = new Bt(e.rand), this.nodeForcesApplicationStrategy = Kt({
			distanceVectorGenerator: this.distanceVectorGenerator,
			nodeCharge: e.nodeCharge,
			maxForce: e.maxForce,
			nodeForceCoefficient: e.nodeForceCoefficient,
	...
	calculateNextCoordinates(e) {
		...
		return new zt(t, i, {
			...
			edgeEquilibriumLength: this.edgeEquilibriumLength,
			edgeStiffness: this.edgeStiffness
		}).apply() < this.convergenceVelocity && !t.getAllNodeIds().some((e) => {
			let n = t.getNode(e);
			return n.x === null || n.y === null;
		}) ? /* @__PURE__ */ new Map() : i;
	}
	...
	switch (e?.type) {
		...
		default: {
			...
			return new nn({
				...
				edgeEquilibriumLength: e?.edgeEquilibriumLength ?? K.edgeEquilibriumLength,
				edgeStiffness: e?.edgeStiffness ?? K.edgeStiffness,
				convergenceVelocity: e?.convergenceVelocity ?? K.convergenceVelocity,
				maxForce: e?.maxForce ?? K.maxForce,
				nodeForceCoefficient: e?.nodeForceCoefficient ?? K.nodeForceCoefficient,
				barnesHutTheta: e?.barnesHut?.theta ?? K.barnesHutTheta,
				barnesHutAreaRadiusThreshold: e?.barnesHut?.areaRadiusThreshold ?? K.barnesHutAreaRadiusThreshold
			});
		}
	}
	...
	switch (e?.type) {
		...
		case "hierarchical": return new Qt({
			...
			transform: fn(e.transform),
			nextLayerNodesResolver: pn(e.nextLayerNodesResolver)
		});
		default: {
			let t = rn(e?.seed ?? K.seed), n = an(t[0], t[1], t[2], t[3]);
			return new Jt({
	...
	let n = bn, r = bn, i = xn();
	return e === "incremental" && (n = i), t === "incremental" && (r = i), typeof e == "number" && (n = yn(e)), typeof t == "number" && (r = yn(t)), typeof e == "function" && (n = e), typeof t == "function" && (r = t), {
		nodesPriorityFn: n,
		edgesPriorityFn: r
	};
}, Cn = (e) => {
	let t = Sn(e.nodes?.priority, e.edges?.priority);
	return {
		nodes: {
...
}, wn = (e) => e.applyOn.type === "topologyChangeSchedule" ? e.applyOn.schedule : R, Tn = (e) => {
	let { canvasDefaults: t } = e, n = e.hasLayout ? wn(e.layoutParams) : R;
	return { focus: {
	...
	build() {
		...
		this.used = !0;
		let e = new y(this.element), t = new h(), n = new Ct(this.element), r = this.createHtmlView(n.main, t, e), i = Cn(this.canvasDefaults), a = new Ue(t, r, i), o = hn(this.layoutConfig), s = new Xe(t, e, Tn({
			canvasDefaults: this.canvasDefaults,
			hasLayout: this.hasLayout,
			layoutParams: o
		}), this.window), c = new Ke(e), l = new d(new He(t), c, a, s);
		if (this.hasBackground && ct.configure(l, Nt(this.backgroundConfig), n.background), this.hasNodeResizeReactiveEdges && b.configure(l), this.userSelectableEdgesConfig !== void 0) {
			let e = On(this.userSelectableEdgesConfig);
			yt.configure(l, this.window, this.pointInsideVerifier, this.eventTagger, e);
		}
		if (this.userSelectableNodesConfig !== void 0) {
			let e = En(this.userSelectableNodesConfig);
			vt.configure(l, this.window, this.pointInsideVerifier, this.eventTagger, e);
		}
		if (this.userSelectableCanvasConfig !== void 0) {
			let e = Dn(this.userSelectableCanvasConfig);
		...
		if (this.hasDraggableNodes) {
			...
			this.hasAnimatedLayout && (e = gn(e, this.animationStaticNodes)), nt.configure(l, n.main, this.window, this.pointInsideVerifier, this.eventTagger, e);
		}
		...
		if (this.virtualScrollConfig === void 0 ? this.hasTransformableViewport && rt.configure(l, n.main, this.window, this.pointInsideVerifier, this.eventTagger, At(this.transformConfig)) : it.configure(l, n.main, this.window, At(this.transformConfig), this.boxRenderingTrigger, this.pointInsideVerifier, this.eventTagger, It(this.virtualScrollConfig)), this.hasLayout && mt.configure(l, o), this.hasAnimatedLayout) {
			let e = cn(this.animatedLayoutConfig);
			this.hasDraggableNodes && (_n(l, this.animationStaticNodes), e = vn(e, this.animationStaticNodes)), _t.configure(l, e, this.window);
		}
...
//#endregion
export { Ee as BezierEdgeShape, kn as CanvasBuilder, Rt as CanvasBuilderError, p as CanvasError, e as ConnectionCategory, I as DirectEdgeShape, l as EventSubject, Oe as HorizontalEdgeShape, ze as InteractiveEdgeError, Be as InteractiveEdgeShape, Ve as MidpointEdgeShape, ke as StraightEdgeShape, Me as VerticalEdgeShape, Ne as boxPortOffsetFn };
